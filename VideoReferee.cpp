...
//  Speed steps
// ============================================================
const double VideoRefereeWindow::kSpeedSteps[] = { 0.125, 0.25, 0.5, 1.0, 2.0, 4.0, 8.0 };
...
static inline uint32_t fromBE32(uint32_t v) { return toBE32(v); }
...
void CaptureWorker::start()
{
...
#endif
    if (!cap.isOpened()) {
        emit errorOccurred(QString("Cannot open camera %1.").arg(m_cameraIndex));
        emit finished();
        return;
    }
    ...
    if (actualFps < 1.0 || actualFps > 120.0) actualFps = 50.0;
    emit fpsDetected(actualFps);

    FILE *outFile = fopen(m_outputPath.toLocal8Bit().constData(), "wb");
    if (!outFile) {
        emit errorOccurred(QString("Cannot open output file: %1").arg(m_outputPath));
        emit finished();
        return;
    }
    ...
    while (m_running) {
        ...
        if (!newPath.isEmpty()) {
            ...
            if (!outFile) {
                emit errorOccurred(QString("Cannot open new file: %1").arg(m_outputPath));
                emit finished();
                return;
            }
        ...
        cv::putText(frame, ts, org, fontFace, scale,
        ...
        cv::resize(rgb, small, cv::Size(640, 360), 0, 0, cv::INTER_LINEAR);
...
void CaptureWorker::stop() { m_running = false; }
...
int VideoRefereeWindow::selectCamera()
{
    ...
    for (int i = 0; i < devices.size(); ++i)
        items << QString("Camera %1: %2").arg(i).arg(devices[i].description());

    bool ok = false;
    QString chosen = QInputDialog::getItem(nullptr, "Select Camera",
                                           "Available cameras:", items, 0, false, &ok);
    if (!ok) return -1;
    ...
    : QMainWindow(parent), m_cameraIndex(cameraIndex)
{
    ...
    connect(camMenu->addAction("&Switch Camera…"),
            &QAction::triggered, this, &VideoRefereeWindow::onSwitchCamera);
    ...
    connect(rotateAct, &QAction::triggered, this, &VideoRefereeWindow::onRotateFiles);
    ...
    connect(m_playbackTimer, &QTimer::timeout,
            this, &VideoRefereeWindow::onPlaybackTimer);

    m_statusTimer = new QTimer(this);
    connect(m_statusTimer, &QTimer::timeout,
            this, &VideoRefereeWindow::onStatusTimer);
...
{
    if (m_replayFile) { fclose(m_replayFile); m_replayFile = nullptr; }
}

void VideoRefereeWindow::startCapture(int cameraIndex, const QString &outputPath)
{
    m_cameraIndex          = cameraIndex;
    ...
    connect(m_captureThread, &QThread::started,
            m_captureWorker, &CaptureWorker::start);
    connect(m_captureWorker, &CaptureWorker::frameCaptured,
            this, &VideoRefereeWindow::onFrameCaptured,  Qt::QueuedConnection);
    connect(m_captureWorker, &CaptureWorker::frameIndexed,
            this, &VideoRefereeWindow::onFrameIndexed,   Qt::QueuedConnection);
    connect(m_captureWorker, &CaptureWorker::fpsDetected,
            this, &VideoRefereeWindow::onFpsDetected,    Qt::QueuedConnection);
    connect(m_captureWorker, &CaptureWorker::rotationDone,
            this, &VideoRefereeWindow::onRotationDone,   Qt::QueuedConnection);
    connect(m_captureWorker, &CaptureWorker::errorOccurred,
            this, &VideoRefereeWindow::onCaptureError,   Qt::QueuedConnection);
...
{
    if (!m_captureWorker || !m_captureThread) return;
...
void VideoRefereeWindow::spawnTranscode(const QString &sourcePath)
{
    ...
    QString batPath = sourcePath + ".transcode.bat";
    {
        ...
        if (bat.open(QIODevice::WriteOnly | QIODevice::Text)) {
            QTextStream s(&bat);
            s << "@echo off\r\n";
            s << QString("ffmpeg -f mjpeg -framerate 50 -i \"%1\" "
    ...
    proc->start("sh", QStringList()
        << "-c"
        << QString("ffmpeg -f mjpeg -framerate 50 -i '%1' "
                   "-c:v libx265 -crf 23 -preset fast -r 50 '%2' "
                   "> '%3' 2>&1 && rm '%1'")
                   .arg(sourcePath).arg(outPath).arg(logPath));
#endif

    updateStatusBar();
}

void VideoRefereeWindow::onTranscodeFinished(int exitCode, QProcess::ExitStatus /*status*/)
{
    ...
    if (m_closingDown) {
        if (m_shutdownDialog) {
            ...
            if (label) {
                if (exitCode == 0)
                    label->setText(QString("Done: %1\nClosing...").arg(QFileInfo(out).fileName()));
                else
                    label->setText(QString("Transcode failed - source .mjpeg kept.\n%1").arg(QFileInfo(src).fileName()));
            }
        }
        if (m_transcodeProcesses.isEmpty()) {
            if (m_shutdownDialog) { m_shutdownDialog->close(); m_shutdownDialog = nullptr; }
            hide();
            QApplication::quit();
        }
        return;
    ...
    if (exitCode == 0)
        m_statusBar->showMessage(QString("Transcode complete: %1").arg(QFileInfo(out).fileName()));
    else
        m_statusBar->showMessage(QString("Transcode FAILED for %1 - source kept, check .log").arg(QFileInfo(src).fileName()));
    updateStatusBar();
}
...
{
    m_liveFrame = image;
    if (m_isLive) { showFrame(image, false); updateStatusBar(); }
}
...
void VideoRefereeWindow::onFpsDetected(double fps) { m_captureFps = fps; }

// ---- Read frame from current recording file ----
QImage VideoRefereeWindow::readFrameAt(qint64 filePos)
{
    if (!m_replayFile) {
        m_replayFile = fopen(m_currentRecordingPath.toLocal8Bit().constData(), "rb");
        if (!m_replayFile) return {};
    }
    if (fseek(m_replayFile, static_cast<long>(filePos), SEEK_SET) != 0) return {};

    uint32_t magic = 0, lenNet = 0;
    if (fread(&magic,  4, 1, m_replayFile) != 1) return {};
    if (fread(&lenNet, 4, 1, m_replayFile) != 1) return {};
    if (fromBE32(magic) != kFrameMagic) return {};

    uint32_t len = fromBE32(lenNet);
    std::vector<uchar> buf(len);
    if (fread(buf.data(), 1, len, m_replayFile) != len) return {};
...
void VideoRefereeWindow::showFrame(const QImage &img, bool overlay)
{
    ...
    if (overlay) {
        ...
        p.setFont(font);
        QRect tr = QFontMetrics(font).boundingRect("REPLAY").adjusted(-8,-4,8,4);
        ...
        p.setFont(font);
        QRect mr = QFontMetrics(font).boundingRect("MARKER SET").adjusted(-8,-4,8,4);
...
{
    int total;
    { QMutexLocker lk(&m_indexMutex); total = static_cast<int>(m_frameIndex.size()); }
    newFrame = qBound(0, newFrame, total - 1);
    m_replayFrame = newFrame;
    qint64 pos;
    { QMutexLocker lk(&m_indexMutex); pos = m_frameIndex[static_cast<size_t>(newFrame)]; }
...
void VideoRefereeWindow::onPlaybackTimer()
{
    ...
    int total;
    { QMutexLocker lk(&m_indexMutex); total = static_cast<int>(m_frameIndex.size()); }
    if (++m_replayFrame >= total) { goLive(); return; }
    qint64 pos;
    { QMutexLocker lk(&m_indexMutex); pos = m_frameIndex[static_cast<size_t>(m_replayFrame)]; }
    showFrame(readFrameAt(pos), true);
    updateStatusBar();
}
...
void VideoRefereeWindow::enterReplay()
{
    if (!m_isLive) return;
    m_isLive = false;
    { QMutexLocker lk(&m_indexMutex); m_replayFrame = static_cast<int>(m_frameIndex.size()) - 1; }
    updatePlaybackTimer();
    updateStatusBar();
}
...
{
    if (m_isLive || m_isPaused) { m_playbackTimer->stop(); return; }
    int ms = static_cast<int>(1000.0 / (m_captureFps * kSpeedSteps[m_speedIndex]));
...
void VideoRefereeWindow::updateStatusBar()
{
    ...
    QString transcodeInfo = transcoding > 0
...
void VideoRefereeWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_Left:  if (m_isLive) enterReplay(); seekTo(m_replayFrame - jump10s); break;
    case Qt::Key_Right: if (m_isLive) enterReplay(); seekTo(m_replayFrame + jump10s); break;

    case Qt::Key_Comma:
        if (m_isLive) enterReplay();
        m_speedIndex = qMax(0, m_speedIndex - 1);
        updatePlaybackTimer(); updateStatusBar(); break;
    ...
    case Qt::Key_Slash:
        if (m_isLive) enterReplay();
        m_isPaused = true; m_playbackTimer->stop();
        seekTo(m_replayFrame + 1); break;
    ...
    case Qt::Key_Z: {
        ...
        showFrame(m_isLive ? m_liveFrame : readFrameAt(m_frameIndex[m_replayFrame]), !m_isLive);
        break;
    }
    case Qt::Key_X:
        if (m_isLive) enterReplay();
...
void VideoRefereeWindow::onSwitchCamera()
{
    ...
    if (devices.isEmpty()) {
        QMessageBox::warning(this, "No cameras", "No cameras found.");
        return;
    }

    QStringList items;
    for (int i = 0; i < devices.size(); ++i)
        items << QString("Camera %1: %2%3")
            .arg(i)
            .arg(devices[i].description())
            .arg(i == m_cameraIndex ? " (current)" : "");


    bool ok = false;
    QString chosen = QInputDialog::getItem(this, "Switch Camera",
                                           "Select camera:", items, 0, false, &ok);
    if (!ok) return;
    int newIdx = items.indexOf(chosen);
    if (newIdx == m_cameraIndex) return;

    stopCapture();
    goLive();
    if (m_replayFile) { fclose(m_replayFile); m_replayFile = nullptr; }
    { QMutexLocker lk(&m_indexMutex); m_frameIndex.clear(); }

    QString path = newRecordingPath();
    startCapture(newIdx, path);
}
...
void VideoRefereeWindow::closeEvent(QCloseEvent *event)
{
    ...
    m_closingDown = true;
    event->ignore(); // never accept here; we call QApplication::quit() ourselves

    // Stop capture — this flushes and closes the current .mjpeg file
    stopCapture();
    if (m_replayFile) { fclose(m_replayFile); m_replayFile = nullptr; }

    // Kill any mid-session transcodes that are still running (warm-up files)
    ...
    m_shutdownDialog = new QDialog(nullptr, Qt::Dialog | Qt::CustomizeWindowHint | Qt::WindowTitleHint);
    m_shutdownDialog->setWindowTitle("Transcoding final recording…");
    m_shutdownDialog->setFixedSize(420, 80);

    QLabel *label = new QLabel(
        QString("Transcoding %1\nPlease wait…")
            .arg(QFileInfo(m_currentRecordingPath).fileName()),
        m_shutdownDialog);
...
void VideoRefereeWindow::initTcpServer()
{
    ...
    connect(m_tcpServer, &QTcpServer::newConnection,
            this, &VideoRefereeWindow::onNewTcpConnection);
...
void VideoRefereeWindow::onNewTcpConnection()
{
    while (m_tcpServer->hasPendingConnections()) {
        ...
        connect(client, &QTcpSocket::readyRead,
                this, &VideoRefereeWindow::onTcpClientData);
        connect(client, &QTcpSocket::disconnected,
                this, &VideoRefereeWindow::onTcpClientDisconnected);
...
void VideoRefereeWindow::handleTcpCommand(QTcpSocket *client, const QString &cmd)
{
    if (cmd == "ROTATE") {
        if (m_closingDown) {
        ...
        if (m_isLive) enterReplay();
        m_speedIndex = qMin(kNumSpeedSteps - 1, m_speedIndex + 1);
        updatePlaybackTimer(); updateStatusBar();
        sendToClient(client, "< OK");
        broadcastStatus();
        ...
        if (!ok || step < 1 || step > kNumSpeedSteps) {
            sendToClient(client, QString("< ERROR speed must be 1-%1").arg(kNumSpeedSteps));
            return;
        }
        if (m_isLive) enterReplay();
        m_speedIndex = step - 1;
        ...
        if (!ok) { sendToClient(client, "< ERROR usage: JUMP +N or JUMP -N"); return; }
        if (m_isLive) enterReplay();
...
{
    if (m_tcpClients.isEmpty()) return;
...
QString VideoRefereeWindow::currentStatusLine() const
{
    ...
    if (m_closingDown) {
        ...
        return QString("# STATUS REPLAY pos=%1 dur=%2 speed=%3 paused=%4 transcoding=%5")
            .arg(toHMS(posSecs))
            .arg(toHMS(recSecs))
            .arg(kSpeedSteps[m_speedIndex])
            .arg(m_isPaused ? 1 : 0)
